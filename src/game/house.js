// The buildable house.
//
// This used to be drawn here — flat rectangles with a hand-rolled texture —
// because the scene backgrounds have houses painted into them and a painted
// house cannot be taken apart. The games need walls, roof and door as separate
// objects that can each be built from a different material and each fly off
// on their own.
//
// There is now real artwork for it: بيت.svg, cut into three groups by
// tools/house-split.mjs (the source has no groups at all, so they are
// recovered from geometry and written back with their measured boxes).
//
// The material a player picks still has to read at a glance, and there is only
// one house. So the same carved planks and shingles are recoloured per
// material — golden for straw, as drawn for wood, cold grey for stone. The
// detail survives, and so does the choice.
