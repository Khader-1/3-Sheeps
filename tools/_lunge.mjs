...
// never strike: let him reach the bottom and lunge
let shot = 0;
for (let i = 0; i < 40; i++) {
  await wait(400);
  const late = await p.evaluate(() => document.body.innerText.includes('أمسكَ'));
  const near = await p.evaluate(() => {
    const g = document.querySelector('#layer-world > g');
    return g && (g.getAttribute('transform') || '').includes('scale');
  });
  if (near || late) {
    await p.screenshot({ path: `${OUT}/lunge-${String(shot++).padStart(2, '0')}.png` });
    if (shot > 6) break;
  }
  if (late && shot > 4) break;
}
console.log(`captured ${shot} lunge frames`, errs.length ? 'ERRORS: ' + errs[0] : '(no page errors)');
